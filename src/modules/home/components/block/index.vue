<template>
  <div class="content-block__wrap">
    <a :href="block.link" target="_blank" class="content-block__link">
      <div class="content-block__img">
        <img :src="'data:image/png;base64,' + block.image" alt="">
      </div>
      <h1 class="content-block__title">{{block.title}}</h1>
      <p class="content-block__descr">{{block.description}}</p>
      <div class="content-block__del">
        <svg @click.prevent="removeBlock(block)" width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
